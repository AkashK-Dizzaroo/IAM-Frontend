...
const ApplicationAccessManagementPage = () => {
    ...
    queryFn: async () => {
      ...
      const usersWithAssignments = await Promise.all(
        users.map(async (user) => {
          try {
            const assignmentsResponse = await userService.getUserAssignments(user._id || user.id)
            return {
              ...user,
  ...
  // Assign user to application mutation
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Application Access Management</h2>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-gray-700">Global Role</th>
                    ...
                    <tr key={user._id || user.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-4 align-top">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            {user.organization && (
                              <div className="text-[10px] text-gray-400 mt-0.5">{user.organization}</div>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 align-top">
                        {user.assignments?.length === 0 ? (
                          <span className="text-xs text-gray-400 italic">No applications assigned</span>
                        ) : (
                              ...
                              >
                                <div className="flex items-center justify-between mb-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-semibold text-xs text-gray-900">
                                      {assignment.application?.name || assignment.application?.appCode || 'Unknown App'}
                                    </span>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      onClick={() => handleRemoveAssignment(user._id || user.id, assignment._id || assignment.id)}
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1 text-[10px] text-gray-500">
                                  ...
                                  {assignment.assignedBy && (
                                    <div className="flex items-center gap-1 truncate" title={typeof assignment.assignedBy === 'object' ? assignment.assignedBy.email : assignment.assignedBy}>
                                  ...
                                  {assignment.resource && (
                                    <div className="flex items-center gap-1 col-span-2">
                                      <span className="font-medium text-gray-400">Resource:</span>
                                      <span className="text-indigo-600">{assignment.resource}</span>
                                    </div>
                                  )}
                                </div>
                              </div>
          ...
          <div className="space-y-4 mt-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  const user = users.find(u => (u._id || u.id) === value)
                  setSelectedUser(user)
                }}
              >
                <SelectTrigger>
...
export default ApplicationAccessManagementPage
