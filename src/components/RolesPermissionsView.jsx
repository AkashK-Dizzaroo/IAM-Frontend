...
const RolesPermissionsView = () => {
  ...
  const [permissionPagination, setPermissionPagination] = useState({ total: 0, pages: 0 });
  ...
  const fetchRoles = async () => {
    try {
      ...
      const response = await roleService.getRoles({
        ...
        application: roleApplicationFilter !== 'all' ? roleApplicationFilter : undefined,
  ...
  const fetchPermissions = async () => {
    try {
      ...
      const response = await permissionService.getPermissions({
        ...
        category: permissionCategoryFilter !== 'all' ? permissionCategoryFilter : undefined,
      });
      
      if (response.success) {
        setPermissions(response.data || []);
        setPermissionPagination(response.pagination || { total: 0, pages: 0 });
      }
    } catch (error) {
      toast({
        title: 'Error',
  ...
  }, [activeTab, roleCurrentPage, roleSearchTerm, roleApplicationFilter, roleLevelFilter, permissionCurrentPage, permissionSearchTerm, permissionCategoryFilter]);
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {roles.map((role) => (
                          <tr key={role._id || role.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <Badge variant="outline">{role.level || 'N/A'}</Badge>
                            </td>
                            <td className="px-4 py-3">
                  ...
                  {rolePagination.pages > 1 && (
                    <div className="px-4 py-3 border-t bg-gray-50 flex items-center justify-between">
                      <div className="text-sm text-gray-600">
                        Showing {((roleCurrentPage - 1) * 25) + 1} to {Math.min(roleCurrentPage * 25, rolePagination.total)} of {rolePagination.total} roles
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setRoleCurrentPage((p) => Math.min(rolePagination.pages, p + 1))}
                          disabled={roleCurrentPage === rolePagination.pages}
                        >
                          Next
          ...
          {categories.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Click a category to filter permissions</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  <Badge
                    variant={permissionCategoryFilter === 'all' ? 'default' : 'outline'}
                    className="cursor-pointer"
                    onClick={() => setPermissionCategoryFilter('all')}
                  >
                    All ({permissions.reduce((sum, cat) => sum + (cat.count || 0), 0)})
                  </Badge>
                  {categories.map((cat) => {
                    ...
                    return (
                      <Badge
                        key={categoryName}
                        variant={permissionCategoryFilter === categoryName ? 'default' : 'outline'}
                        className="cursor-pointer"
                        onClick={() => {
              ...
              {permissionsLoading && permissions.length === 0 ? (
                ...
                <>
                        ...
                        {permissions.map((permission) => (
                          <tr key={permission._id || permission.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3">
                              <div>
                                <div className="text-sm font-medium text-gray-900">
                                  {permission.displayName || formatPermissionName(permission.name)}
                                </div>
                            ...
                            <td className="px-4 py-3">
                              <Badge variant="outline">{permission.level || 'N/A'}</Badge>
                            </td>
                            <td className="px-4 py-3">
                  ...
                  {permissionPagination.pages > 1 && (
                    <div className="px-4 py-3 border-t bg-gray-50 flex items-center justify-between">
                      <div className="text-sm text-gray-600">
                        Showing {((permissionCurrentPage - 1) * 25) + 1} to {Math.min(permissionCurrentPage * 25, permissionPagination.total)} of {permissionPagination.total} permissions
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setPermissionCurrentPage((p) => Math.max(1, p - 1))}
                        ...
                        <div className="text-sm text-gray-600">
                          Page {permissionCurrentPage} of {permissionPagination.pages}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setPermissionCurrentPage((p) => Math.min(permissionPagination.pages, p + 1))}
                          disabled={permissionCurrentPage === permissionPagination.pages}
                        >
                          Next
                          <ChevronRight className="w-4 h-4" />
                        </Button>
