...
const UserManagementTable = () => {
  ...
  useEffect(() => {
    fetchUsers();
  }, [currentPage, itemsPerPage, searchTerm, statusFilter, organizationFilter]);

  // Get unique organizations for filter
  const uniqueOrganizations = useMemo(() => {
    const orgs = [...new Set(users.map(user => user.organization).filter(Boolean))].sort();
    return orgs;
  }, [users]);
                      ...
                      <tr key={user._id || user.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {getStatusIcon(user.status || user.isActive ? 'ACTIVE' : 'INACTIVE')}
                            <span className={`px-2 py-1 text-xs rounded-full ${getStatusBadgeVariant(user.status || (user.isActive ? 'ACTIVE' : 'INACTIVE'))}`}>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete {getUserName(userToDelete)}? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
